 ...
 */
BME280_Data BME280_Measure(void) {
	BME280_Data data;
	...
	if (BMEReadRaw() == 0) {
		if (tRaw == 0x800000)
			...
			data.temperature = (BME280_compensate_T_int32(tRaw)) / 100.0;

		if (pRaw == 0x800000)
			data.pressure = 0;
		else {
#if SUPPORT_64BIT
			data.pressure = (BME280_compensate_P_int64(pRaw)) / 256.0;

#elif SUPPORT_32BIT
            data.pressure = (BME280_compensate_P_int32(pRaw));
		...
		if (hRaw == 0x8000)
			...
			data.humidity = (bme280_compensate_H_int32(hRaw)) / 1024.0;
	} else {
